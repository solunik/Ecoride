...
class Covoit {
    ...
    if (strlen($depart) < 1 || strlen($depart) > 40) {
        $_SESSION['errorMessage'] = "'Départ' doit contenir entre 1 et 40 caractères.";
        header("Location: index.php?page=recherche");
        exit();
    }
    if (strlen($arrivee) < 1 || strlen($arrivee) > 40) {
        $_SESSION['errorMessage'] = "'Arrivée' doit contenir entre 1 et 40 caractères.";
        header("Location: index.php?page=recherche");
        exit();
    }

    // Validation caractères autorisés (lettres, espaces, tirets, apostrophes)
    if (!preg_match('/^[\p{L} \'\-]+$/u', $depart)) {
        $_SESSION['errorMessage'] = "'Départ' contient des caractères non autorisés.";
        header("Location: index.php?page=recherche");
        exit();
    }
    if (!preg_match('/^[\p{L} \'\-]+$/u', $arrivee)) {
        $_SESSION['errorMessage'] = "'Arrivée' contient des caractères non autorisés.";
        header("Location: index.php?page=recherche");
        exit();
    }
    ...
    try {
        ...
        $dateFormatted = $dateDepart ? $dateDepart->format('Y-m-d') : $details['date_depart'];
